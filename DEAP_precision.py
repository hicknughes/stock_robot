...
def evaluate_features(stock_df, indicator_function, parameters, temporal_granularity):
    ...
    if temporal_granularity in tb.daily_granularities:
        df_wgrowth = stock_df.groupby('day').apply(lambda x: ind.single_day_growth_calc(x.reset_index(drop=True), lag=parameters['lag'], growth_threshold=parameters['growth_threshold'], temporal_granularity=temporal_granularity)) # Calculate outcome variable
        non_indicator_columns = df_wgrowth.columns #identify all columns before indicator calculation 
        param_it_df = ind.daily_indicator(df_wgrowth.reset_index(drop=True), indicator_function, remaining_params) #Generate indicator column
    elif (temporal_granularity in tb.overnight_granularities) and (temporal_granularity != 'one_day'):
        df_wgrowth = ind.growth_calc_on_close(stock_df, parameters['lag'], parameters['growth_threshold'], temporal_granularity=temporal_granularity) # Calculate outcome variable
        non_indicator_columns = df_wgrowth.columns #identify all columns before indicator calculation 
        param_it_df = indicator_function(df_wgrowth, **remaining_params) #Generate indicator column
    elif (temporal_granularity == 'one_day'):
        df_wgrowth = ind.oneday_growth_calc(stock_df, parameters['lag'], parameters['growth_threshold'], temporal_granularity=temporal_granularity) # Calculate outcome variable
        non_indicator_columns = df_wgrowth.columns #identify all columns before indicator calculation 
        param_it_df = indicator_function(df_wgrowth, **remaining_params) #Generate indicator column
    
    # non_indicator_columns = non_indicator_columns.difference(['day'])
    # non_indicator_columns = pd.Index(non_indicator_columns)
    param_it_df = param_it_df.dropna()
...
def DEAP_clean(tkr, stock_df, indicator_function, parameter_ranges, temporal_granularity, population_scalar=23, max_generations=1, fix_lag_growth=False, assigned_lag=12, assigned_growth_thresh=.001,use_apex=True, use_apex_genesis=False):
    '''
    Parameters
    ----------
    tkr : TYPE string
    ...
    stock_df : TYPE pandas.DataFrame
        DESCRIPTION. A dataframe of stock data prepared by clean_features() in optimization_pipeline.py
    ...
    growth_threshold_minimum = tb.lag_growth_thresh_tiers[temporal_granularity]['growth_threshold'][0] # Establishing a floor for growth_threshold parameter mutations
    
    start_time = time.time() # Optimization duration timekeeping
    
    if (use_apex == True) and (use_apex_genesis == False):
        ...
        try:# In case record doesn't exist...
            ...
            if not apex_df.empty: # If the record exists and is not empty...
                ...
                # Re-assign true number of apex individuals pulled
                num_apex = len(apex_individuals)
    ...
    elif use_apex_genesis == True: # A first time round for a given stock at a given temporal_granularity
    ...
    for j in range(len(population)):
        ...
        try:
            fitness = toolbox.evaluate(stock_df, indicator_function, population[j], temporal_granularity)
            population[j].fitness.values = fitness
        except KeyboardInterrupt:
            return
        except Exception as e:
            individual = population[j]
            population[j].fitness.values = (0, 0, 0, 0)
    ...
    while generation < max_generations: #Starting the evolutionary process stepping off from generation 0
        ...
        for k in range(len(offspring)):
            try:
                fitness = toolbox.evaluate(stock_df, indicator_function, offspring[k], temporal_granularity)
                offspring[k].fitness.values = fitness
            except KeyboardInterrupt:
                return
            except Exception as e:
                individual = population[k]
                population[k].fitness.values = (0, 0, 0, 0)
                print(f'{Fore.RED}Indicator calculation error for {str(indicator_function).strip().split()[1]} with individual: {individual}{Style.RESET_ALL}')
                print(f'Error message: {str(e)}')
